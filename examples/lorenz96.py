"""
Example: Lorenz 96 System
Demonstrates the resolvent learning framework for a 6D system.
"""
...
F = 0.1  # Forcing parameter

# Define symbolic variables (must match dimension)
x1, x2, x3, x4, x5, x6 = sympy.symbols('x1 x2 x3 x4 x5 x6')

# Define dynamics as symbolic expressions for Lorenz 96
# f[i] represents dx_i/dt
f_symbolic = [
    -x1 + F,                                    # dx1/dt (k=0)
    x1 * x3 - x2 + F,                          # dx2/dt (k=1)
    -x1 * x2 + x2 * x4 - x3 + F,              # dx3/dt (k=2)
    -x2 * x3 + x3 * x5 - x4 + F,              # dx4/dt (k=3)
    -x3 * x4 + x4 * x6 - x5 + F,              # dx5/dt (k=4)
    -x4 * x5 - x6 + F                          # dx6/dt (k=5, N-1)
]

# System name (will be used for saving files)
SYSTEM_NAME = '96Lorenz'
...
DIM = len(f_symbolic)
SYMBOLS = [x1, x2, x3, x4, x5, x6][:DIM]  # Extract only the symbols needed

# Convert symbolic expressions to numerical function
def ode_function(t, var):
    """
    ODE function automatically generated from symbolic expressions.
    
    Args:
        t: Time (not used in autonomous systems)
        var: State vector [x1, x2, x3, x4, x5, x6]
    
    Returns:
        Derivative vector [dx1/dt, dx2/dt, ..., dx6/dt]
    """
...
M_FOR_1D = 2  # Number of samples per dimension
X_LIM = 1  # Spatial limit for initial conditions
SPAN = 1  # Time span
GEN_FREQUENCY = 1000  # High frequency for data generation
SAVE_FREQUENCIES = [10, 50, 100]  # Frequencies to save

# Learning parameters
FREQUENCY = 100  # Frequency to use for learning
MONOMIAL_DEGREES = [2, 2, 2, 2, 2, 2]  # Polynomial degrees for each dimension
